...
impl Layer {
  fn create(idx: usize, fan_in: usize, fan_out: usize, activation: Activation, device: &Device) -> Result<Self> {
    ...
    let layer = Self { idx, weights, biases, activation, debug: true };
  ...
  fn backward(&mut self, dz: &Tensor, a: &Tensor, m: f64, learning_rate: f64, is_last: bool) -> Result<Tensor> {
    if self.debug {
    ...
    let dz = if is_last {
      ...
      self.weights.t()?.matmul(&dz)?.mul(&self.get_activation_derivative(a)?)?
    };
  ...
  fn update(&mut self, dw: &Tensor, db: &Tensor, learning_rate: f64) -> Result<()> {
    self.weights = self.weights.sub(&dw.broadcast_mul(&Tensor::new(learning_rate, dw.device())?)?)?;
    self.biases = self.biases.sub(&db.broadcast_mul(&Tensor::new(learning_rate, db.device())?)?)?;
...
impl Network {
  ...
  pub fn add_layer(&mut self, nb_neuron: usize, activation: Activation) -> &mut Self {
    ...
    self.activations.insert(self.activations.len() - 1, activation);
  ...
  pub fn build(&mut self) -> Result<&mut Self> {
    ...
    for idx in 1..len {
      ...
      self.layers.push(Layer::create(idx, fan_in, fan_out, activation, &self.device)?);
    }
  ...
  fn get_accuracy(&self, x: &Tensor, y: &Tensor) -> Result<f64> {
    ...
    Ok(x.eq(&y)?.to_dtype(DType::F64)?.sum_all()?.to_scalar::<f64>()? / batch)
  }

  fn get_loss(&self, a: &Tensor, y: &Tensor) -> Result<f64> {
    cross_entropy(&a, &y.t()?.argmax(0)?)?.to_dtype(DType::F64)?.to_scalar()
  }

  pub fn predict(&self, x: &Tensor) -> Result<Tensor> {
    self.forward(x)?.last().unwrap().argmax(0)?.to_dtype(DType::F64)
  }

  pub fn train(&mut self, x: &Tensor, y: &Tensor, learning_rate: f64, nb_iter: usize) -> Result<(Vec<f64>, Vec<f64>)> {
    let step = max(1, nb_iter / 100);
    let quiet = !self.debug;
    println!("Training network with {} iterations, save step each {}", nb_iter, step);
